...
class AdminDamageProductService extends AdminAbstractServices {
  ...
  public async addDamageProductService(req: Request) {
    const { p_damage, pd_product, pdp_attribute } = req.body;
    // check warehouse
    ...
    return await this.db.transaction(async (trx) => {
      ...
      if (pdp_attribute) {
        ...
        for (let h = 0; h < pd_product.length; h++) {
          for (let i = 0; i < pdp_attribute.length; i++) {
            if (pd_product[h].pdp_p_id === pdp_attribute[i].pdpa_pdp_id) {
              ptp_attributeVal.push({
                pdpa_av_id: pdp_attribute[i].pdpa_av_id,
                pdpa_quantity: pdp_attribute[i].pdpa_quantity,
        ...
        await trx('pdp_attribute').insert(ptp_attributeVal);
      }
  ...
  public async getSingleDamageProductService(req: Request) {
    ...
    const data = await this.db('pd_product AS pdp')
      ...
      .leftJoin('pdp_attribute AS pda', 'pdp.pdp_id', 'pda.pdpa_pdp_id')
      .leftJoin('attribute_value AS av', 'pda.pdpa_av_id', 'av.av_id')
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      for (let j = 0; j < data2.length; j++) {
        if (data[i].p_id === data[j].p_id) {
          ...
          data2[j].pdp_attribute.push({
            pdpa_id: data[i].pdpa_id,
            pdpa_av_id: data[i].pdpa_av_id,
  ...
  public async updateDamageProductService(req: Request) {
    const { id } = req.params;
    const { pdp_product, pdp_attribute } = req.body;
    ...
    return await this.db.transaction(async (trx) => {
      ...
      if (pdp_attribute) {
        const { pdpa_id, ...restAtrb } = pdp_attribute;
        res = await trx('pdp_attribute AS pdpa')
          .update(restAtrb)
          .where('pdpa.pdpa_id', pdp_attribute.pdpa_id);
      }
